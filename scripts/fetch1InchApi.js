...
module.exports = async function (callback) {
    ...
    const usdt = tokensArray.filter(obj => {
        return obj.symbol === 'USDT'
    })
    const usdc = tokensArray.filter(obj => {
        return obj.symbol === 'USDC'
    })
    const dai = tokensArray.filter(obj => {
        return obj.symbol === 'DAI'
    })


    const supplyAmount = 1000;
    const selectedToken = tokensArray.filter(obj => {
    ...
    fetchPricesForward(selectedToken[0].address);
    ...
    async function fetchPricesForward(_tokenAddress) {
        ...
        var maxParsed = null;
        const toUSDT = await axios.get('https://api.1inch.io/v4.0/1/quote?fromTokenAddress=' + _tokenAddress + '&toTokenAddress=' + USDTAddress + '&amount=' + formattedSupplyAmount).then(res => {
        ...
        const toUSDC = await axios.get('https://api.1inch.io/v4.0/1/quote?fromTokenAddress=' + _tokenAddress + '&toTokenAddress=' + USDCAddress + '&amount=' + formattedSupplyAmount).then(res => {
        ...
        const toDAI = await axios.get('https://api.1inch.io/v4.0/1/quote?fromTokenAddress=' + _tokenAddress + '&toTokenAddress=' + DAIAddress + '&amount=' + formattedSupplyAmount).then(res => {
        ...
        const firstSwap = await fetch('https://api.1inch.io/v4.0/1/swap?fromTokenAddress='+selectedToken[0].address+'&toTokenAddress='+maxReturnTokenAddress+'&amount='+formattedSupplyAmount+'&fromAddress='+signerAddresses[0]+'&slippage=1&disableEstimate=true').then(res => res.json()).then(res => {
            console.log(res.tx)
